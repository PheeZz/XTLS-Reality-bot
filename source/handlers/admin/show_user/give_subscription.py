...
async def ask_admin_for_subscription_duration(call: types.CallbackQuery, state: FSMContext):
...
async def give_subscription_to_user_and_notify_him(message: types.Message, state: FSMContext):
    data = await state.get_data()
    user_id: int = data.get("user_id")
    subscription_duration: int = data.get("subscription_duration")
    await db_manager.add_days_to_user_subscription(user_id=user_id, days=subscription_duration)
    try:
